...
StringData::~StringData() { delete d_ptr; }

// Add new empty row
void StringData::addEmptyRow() { d_ptr->m_values << QStringList(); }

// Add new row with one value
// @input:
// - value - value that is supposed to be written to the new row
void StringData::addRow(const QString &value) { d_ptr->m_values << (QStringList() << value); }

// Add new row with specified values (as strings)
// @input:
// - values - list of strings. If list is empty, it will be interpreted
// as empty line
void StringData::addRow(const QStringList &values) { d_ptr->m_values << values; }

// Clear all data
void StringData::clear() { d_ptr->m_values.clear(); }
...
// - value - value that is supposed to be written to the new row
void StringData::insertRow(const int &row, const QString &value) { insertRow(row, (QStringList() << value)); }
...
void StringData::insertRow(const int &row, const QStringList &values) { d_ptr->m_values.insert(row, values); }

// Check if there are any rows
// @output:
// - bool - True if there are any rows, else False
bool StringData::isEmpty() const { return d_ptr->m_values.isEmpty(); }
...
void StringData::removeRow(const int &row) { d_ptr->m_values.removeAt(row); }
...
void StringData::replaceRow(const int &row, const QString &value) { replaceRow(row, (QStringList() << value)); }
...
// 'old' values
void StringData::replaceRow(const int &row, const QStringList &values) { d_ptr->m_values.replace(row, values); }
...
void StringData::reserve(const int &size) { d_ptr->m_values.reserve(size); }

// Get number of rows
// @output:
// - int - current number of rows
int StringData::rowCount() const { return d_ptr->m_values.size(); }
...
bool StringData::operator==(const StringData &other) const { return d_ptr->m_values == other.d_ptr->m_values; }
