...
ContentIterator::ContentIterator(const AbstractData &data, const QString &separator, const QString &textDelimiter,
                                 const QStringList &header, const QStringList &footer, const int chunkSize)
    : m_data(data), m_separator(separator), m_textDelimiter(textDelimiter), m_header(header), m_footer(footer),
...
bool ContentIterator::isEmpty() const { return m_data.isEmpty() && m_header.isEmpty() && m_footer.isEmpty(); }
...
bool ContentIterator::hasNext() const { return !atEnd; }
...
QString ContentIterator::composeRow(const QStringList &values) const {
    ...
    for (int i = 0; i < rowValues.size(); ++i) {
        ...
        if (delimiter.isEmpty() &&
            (rowValues.at(i).contains(m_separator) || rowValues.at(i).contains(CR) || rowValues.at(i).contains(LF))) {
