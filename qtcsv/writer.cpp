...
class WriterPrivate {
  public:
    ...
    static bool appendToFile(const QString &filePath, ContentIterator &content, QTextCodec *codec);

    // Overwrite file with new information
    static bool overwriteFile(const QString &filePath, ContentIterator &content, QTextCodec *codec);

    // Write to IO Device
    static bool writeToIODevice(QIODevice &ioDevice, ContentIterator &content, QTextCodec *codec);
...
bool WriterPrivate::appendToFile(const QString &filePath, ContentIterator &content, QTextCodec *codec) {
...
bool WriterPrivate::overwriteFile(const QString &filePath, ContentIterator &content, QTextCodec *codec) {
...
bool WriterPrivate::writeToIODevice(QIODevice &ioDevice, ContentIterator &content, QTextCodec *codec) {
    if (content.isEmpty()) {
        qDebug() << __FUNCTION__ << "Error - invalid arguments";
        return false;
    }
    ...
    if (false == ioDevice.isOpen() && false == ioDevice.open(QIODevice::Append | QIODevice::Text)) {
...
QString WriterPrivate::getTempFileName() {
    QString nameTemplate =
        QDir::tempPath() + "/qtcsv_" + QString::number(QCoreApplication::applicationPid()) + "_%1.csv";
...
bool Writer::write(const QString &filePath, const AbstractData &data, const QString &separator,
                   const QString &textDelimiter, const WriteMode &mode, const QStringList &header,
                   const QStringList &footer, QTextCodec *codec) {
    if (filePath.isEmpty()) {
...
bool Writer::write(QIODevice &ioDevice, const AbstractData &data, const QString &separator,
                   const QString &textDelimiter, const QStringList &header, const QStringList &footer,
                   QTextCodec *codec) {
    if (data.isEmpty()) {
        qDebug() << __FUNCTION__ << "Error - empty data";
        return false;
