...
class ReaderPrivate {
  public:
    ...
    static bool read(QIODevice &ioDevice, Reader::AbstractProcessor &processor, const QString &separator,
  ...
  private:
    ...
    static QStringList splitElements(const QString &line, const QString &separator, const QString &textDelimiter,
                                     ElementInfo &elemInfo);

    // Try to find end position of first or middle element
    static int findMiddleElementPosition(const QString &str, const int &startPos, const QString &separator,
                                         const QString &txtDelim);

    // Check if current element is the last element
    static bool isElementLast(const QString &str, const int &startPos, const QString &separator,
                              const QString &txtDelim);
...
bool ReaderPrivate::read(QIODevice &ioDevice, Reader::AbstractProcessor &processor, const QString &separator,
                         const QString &textDelimiter, QTextCodec *codec) {
...
QStringList ReaderPrivate::splitElements(const QString &line, const QString &separator, const QString &textDelimiter,
                                         ElementInfo &elemInfo) {
    ...
    while (pos < line.size()) {
        if (elemInfo.isEnded) {
            ...
            if (delimiterPos == pos) {
                ...
                // other.
                int midElemEndPos = findMiddleElementPosition(line, pos, separator, textDelimiter);
                if (midElemEndPos > 0) {
...
// or middle
int ReaderPrivate::findMiddleElementPosition(const QString &str, const int &startPos, const QString &separator,
                                             const QString &txtDelim) {
    const int ERROR = -1;
...
// False otherwise
bool ReaderPrivate::isElementLast(const QString &str, const int &startPos, const QString &separator,
                                  const QString &txtDelim) {
    if (str.isEmpty() || startPos < 0 || separator.isEmpty() || txtDelim.isEmpty()) {
        return false;
    }
...
void ReaderPrivate::removeExtraSymbols(QStringList &elements, const QString &textDelimiter) {
    ...
    for (int i = 0; i < elements.size(); ++i) {
        ...
        if (false == textDelimiter.isEmpty()) {
            ...
            QStringRef strEnd(&elements.at(i), endPos - textDelimiter.size() + 1, textDelimiter.size());
...
QList<QStringList> Reader::readToList(const QString &filePath, const QString &separator, const QString &textDelimiter,
                                      QTextCodec *codec) {
    QFile file;
...
QList<QStringList> Reader::readToList(QIODevice &ioDevice, const QString &separator, const QString &textDelimiter,
                                      QTextCodec *codec) {
...
bool Reader::readToData(const QString &filePath, AbstractData &data, const QString &separator,
                        const QString &textDelimiter, QTextCodec *codec) {
    QFile file;
    if (false == openFile(filePath, file)) {
        return false;
...
bool Reader::readToData(QIODevice &ioDevice, AbstractData &data, const QString &separator, const QString &textDelimiter,
                        QTextCodec *codec) {
    ReadToListProcessor processor;
    const bool result = ReaderPrivate::read(ioDevice, processor, separator, textDelimiter, codec);
    if (result) {
...
// @output:
// - bool - True if file was successfully read, otherwise False
bool Reader::readToProcessor(const QString &filePath, Reader::AbstractProcessor &processor, const QString &separator,
                             const QString &textDelimiter, QTextCodec *codec) {
    QFile file;
    if (false == openFile(filePath, file)) {
        return false;
...
bool Reader::readToProcessor(QIODevice &ioDevice, Reader::AbstractProcessor &processor, const QString &separator,
                             const QString &textDelimiter, QTextCodec *codec) {
