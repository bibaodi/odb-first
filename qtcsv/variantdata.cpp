...
bool VariantData::VariantDataPrivate::isConvertableToString(const QList<QVariant> &values) const {
    for (QList<QVariant>::const_iterator iter = values.constBegin(); iter != values.constEnd(); ++iter) {
...
QList<QVariant> VariantData::VariantDataPrivate::toListOfVariants(const QStringList &values) const {
    QList<QVariant> list;
    for (QStringList::const_iterator iter = values.constBegin(); iter != values.constEnd(); ++iter) {
        list << QVariant(*iter);
    }
...
VariantData::~VariantData() { delete d_ptr; }

// Add new empty row
void VariantData::addEmptyRow() { d_ptr->m_values << QList<QVariant>(); }
...
void VariantData::addRow(const QStringList &values) { d_ptr->m_values << d_ptr->toListOfVariants(values); }

// Clear all data
void VariantData::clear() { d_ptr->m_values.clear(); }
...
bool VariantData::isEmpty() const { return d_ptr->m_values.isEmpty(); }
...
void VariantData::removeRow(const int &row) { d_ptr->m_values.removeAt(row); }
...
void VariantData::reserve(const int &size) { d_ptr->m_values.reserve(size); }

// Get number of rows
// @output:
// - int - current number of rows
int VariantData::rowCount() const { return d_ptr->m_values.size(); }
...
bool VariantData::operator==(const VariantData &other) const { return d_ptr->m_values == other.d_ptr->m_values; }
